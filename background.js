...
async function getTeamInfoFromCookies() {
  try {
    ...
    // Look for cookie with team ID
    const teamCookie = cookies.find(cookie => 
      cookie.name === 'wispersisted');
    
    if (!teamCookie) {
      console.log('No team cookie found');
      return null;
    }
    
    // Extract team ID from cookie
    let teamId;
    if (teamCookie.value.includes('gd_teamid=')) {
      // Extract the team ID using regex to capture exactly 5 digits after gd_teamid=
      const match = teamCookie.value.match(/gd_teamid=(\d{5})\b/);
      if (match && match[1]) {
      teamId = match[1];
      } else {
      console.log('Team ID not found in expected format (should be 5 digits)');
      return null;
      }
    } else {
      console.log('gd_teamid not found in cookie');
      return null;
    }
    
    console.log(`Found team ID in cookies: ${teamId}`);
