...
export async function initializeDefaultRatings() {
  ...
  try {
    // Check if defaults are already stored
    const storedDefaults = await recruitStorage.getConfig('defaultRoleRatings');
    
    if (!storedDefaults) {
      console.log('Initializing default role ratings in storage');
      ...
      await recruitStorage.saveConfig('defaultRoleRatings', JSON.stringify(defaultData.roleRatings));
      console.log('Default role ratings successfully stored in database');
    }
...
async function loadRoleRatings() {
  try {
    ...
    // Priority 1: Check for custom role ratings
    ...
    // Priority 2: Use stored default ratings
    const storedDefaults = await recruitStorage.getConfig('defaultRoleRatings');
    
    if (storedDefaults) {
      console.log('Using stored default role ratings');
    ...
    // Store these as defaults for next time
    await recruitStorage.saveConfig('defaultRoleRatings', JSON.stringify(defaultData.roleRatings));
...
export async function saveRoleRatings(customRatings) {
  ...
  try {
    ...
    // Save to storage
    await recruitStorage.saveConfig('customRoleRatings', JSON.stringify(ratingsToSave));
    
    // Force cache invalidation to ensure fresh load on next calculation
    role_ratings_cache = null;
    role_ratings_cache_source = null;
    
    console.log('Custom role ratings saved successfully and cache invalidated');
    return { success: true };
...
export async function resetRoleRatingsToDefaults() {
  ...
  try {
    ...
    console.log('Role ratings reset to defaults successfully and cache cleared');
    return { success: true };
