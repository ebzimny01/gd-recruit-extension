...
// Recalculate role ratings for a specific team (used for cross-team operations)
export async function recalculateRoleRatingsForTeam(teamId, teamInfo, changedPositions = null) {
  console.log(`Recalculating role ratings for team ${teamId} (${teamInfo?.schoolName || 'Unknown'})`, 
    changedPositions ? `(positions: ${changedPositions.join(', ')})` : '(all positions)');
  ...
  try {
    ...
    // Switch to the target team context
    await multiTeamStorage.setActiveTeam(teamId, teamInfo);
    
    // Get all recruits for this team
    const recruits = await multiTeamStorage.getAllRecruits();
    
    if (!Array.isArray(recruits) || recruits.length === 0) {
      console.log(`No recruits found for team ${teamId}`);
      ...
      return { updatedCount: 0, totalRecruits: 0, teamId, schoolName: teamInfo?.schoolName };
    }
    
    let updatedCount = 0;
    const batchSize = 25; // Smaller batches for cross-team operations
    const errors = [];
    
    // Filter recruits that need updates based on changed positions
    const recruitsToUpdate = changedPositions && changedPositions.length > 0 
      ? recruits.filter(recruit => {
          const recruitPos = (recruit.pos || '').toUpperCase();
          return changedPositions.includes(recruitPos);
        })
      : recruits;
    
    console.log(`Team ${teamId}: Processing ${recruitsToUpdate.length} of ${recruits.length} recruits`);
    
    for (let i = 0; i < recruitsToUpdate.length; i += batchSize) {
      const batch = recruitsToUpdate.slice(i, i + batchSize);
      ...
      for (const recruit of batch) {
        try {
          ...
          console.error(`Error recalculating ratings for recruit ${recruit.id} in team ${teamId}:`, error);
          errors.push({ 
      ...
      // Log progress for larger teams
      if (recruitsToUpdate.length > 100 && i % (batchSize * 4) === 0) {
        console.log(`Team ${teamId}: Processed ${Math.min(i + batchSize, recruitsToUpdate.length)} of ${recruitsToUpdate.length} recruits`);
      }
    }
    
    if (errors.length > 0) {
      console.warn(`Team ${teamId}: Encountered ${errors.length} errors during recalculation:`, errors.slice(0, 3));
    }
    
    console.log(`Team ${teamId}: Successfully recalculated role ratings for ${updatedCount} recruits`);
    
    // Restore original team context if it was different
    if (originalTeamId && originalTeamId !== teamId) {
      await multiTeamStorage.setActiveTeam(originalTeamId);
      console.log(`Restored team context to ${originalTeamId}`);
    }
    
    return { 
      updatedCount, 
      totalRecruits: recruits.length, 
      recruitsProcessed: recruitsToUpdate.length,
      teamId, 
      schoolName: teamInfo?.schoolName || 'Unknown',
      errors 
    };
    
  } catch (error) {
    console.error(`Error during role ratings recalculation for team ${teamId}:`, error);
    
    // Try to restore original team context on error
    try {
      const originalTeamId = multiTeamStorage.getCurrentTeamId();
      if (originalTeamId && originalTeamId !== teamId) {
        await multiTeamStorage.setActiveTeam(originalTeamId);
      ...
      console.error('Error restoring team context:', restoreError);
    }
