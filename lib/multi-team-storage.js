...
class MultiTeamRecruitStorage {
  ...
  // Donation reminder tracking methods
  async saveDonationConfig(key, value) {
    console.log(`Saving donation config: ${key} = ${value}`);
    return await this.saveGlobalConfig(`donation_${key}`, value);
  }
  
  async getDonationConfig(key) {
    const result = await this.getGlobalConfig(`donation_${key}`);
    console.log(`Retrieved donation config: ${key} = ${result}`);
    return result;
  }
  
  async shouldShowDonationReminder() {
    try {
      const installDate = await this.getDonationConfig('installDate');
      const lastShown = await this.getDonationConfig('lastReminderShown');
      
      const now = Date.now();
      const thirtyDaysMs = 30 * 24 * 60 * 60 * 1000; // 30 days in milliseconds
      
      // First time user - set install date if not exists
      if (!installDate) {
        console.log('Setting donation install date for new user');
        await this.saveDonationConfig('installDate', now);
        return false; // Don't show reminder immediately
      }
      
      // Check if 30 days have passed since install
      const daysSinceInstall = (now - installDate) / (24 * 60 * 60 * 1000);
      if (daysSinceInstall < 30) {
        console.log(`Only ${Math.floor(daysSinceInstall)} days since install, not showing reminder`);
        return false;
      }
      
      // Check if 30 days have passed since last reminder
      if (lastShown) {
        const daysSinceLastShown = (now - lastShown) / (24 * 60 * 60 * 1000);
        if (daysSinceLastShown < 30) {
          console.log(`Only ${Math.floor(daysSinceLastShown)} days since last reminder, not showing again`);
          return false;
        }
      }
      
      console.log('Donation reminder should be shown');
      return true;
      
    } catch (error) {
      console.error('Error checking donation reminder status:', error);
      return false;
    }
  }
  
  async markDonationReminderShown() {
    try {
      const now = Date.now();
      await this.saveDonationConfig('lastReminderShown', now);
      
      // Increment reminder count
      const currentCount = await this.getDonationConfig('reminderCount') || 0;
      await this.saveDonationConfig('reminderCount', currentCount + 1);
      
      console.log(`Donation reminder marked as shown (count: ${currentCount + 1})`);
      return true;
    } catch (error) {
      console.error('Error marking donation reminder as shown:', error);
      return false;
    }
  }
  ...
  async getDonationStats() {
    try {
      const installDate = await this.getDonationConfig('installDate');
      const lastShown = await this.getDonationConfig('lastReminderShown');
      const reminderCount = await this.getDonationConfig('reminderCount') || 0;
      
      return {
        installDate,
        lastShown,
        reminderCount,
        daysSinceInstall: installDate ? Math.floor((Date.now() - installDate) / (24 * 60 * 60 * 1000)) : 0,
        daysSinceLastShown: lastShown ? Math.floor((Date.now() - lastShown) / (24 * 60 * 60 * 1000)) : null
      };
    } catch (error) {
  ...
  async recordDonationAction(action) {
    try {
      console.log(`Recording donation action: ${action}`);
      
      // Mark reminder as shown
      await this.markDonationReminderShown();
      
      // Record the specific action taken
      await this.saveDonationConfig('lastAction', action);
      await this.saveDonationConfig('lastActionTime', Date.now());
      
      // If user supported, record that they've donated
      if (action === 'support') {
        await this.saveDonationConfig('hasDonated', true);
        await this.saveDonationConfig('donationDate', Date.now());
      }
      
      console.log(`Donation action '${action}' recorded successfully`);
      return true;
